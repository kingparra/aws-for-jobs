...
# The following resources were created with the web up and then imported
...
resource "aws_route_table_association" "midterm-rtb-public-rtbassoc-07944fea09c51bc54" {
  ...
  subnet_id      = aws_subnet.subnet-0b5472b288c48ac87-midterm-subnet-public2-us-east-1b.id
}

resource "aws_route_table_association" "midterm-rtb-public-rtbassoc-02b1538704df4e32a" {
  route_table_id = aws_route_table.midterm-rtb-public.id
  subnet_id      = aws_subnet.subnet-05b228d4a4578782d-midterm-subnet-public1-us-east-1a.id
}
